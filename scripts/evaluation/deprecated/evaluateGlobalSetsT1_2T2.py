...
def getMeasures(d):
    ...
    fp=d["type1_2_false"]
    fn=d["type2_false"]
    if((d["type1_2_ok"]+d["type1_2_false"]))>0.0:
        ppv=d["type1_2_ok"]/(d["type1_2_ok"]+d["type1_2_false"])
    else:
        ppv=0.0
    if (d["type1_2_ok"]+d["type2_false"])>0.0:
        tpr=d["type1_2_ok"]/(d["type1_2_ok"]+d["type2_false"])
    else:
        tpr=0.0
    if (d["type2_ok"]+d["type1_2_false"])>0.0:
        tnr=d["type2_ok"]/(d["type2_ok"]+d["type1_2_false"])
    else:
        tnr=0.0
    ba=(tpr+tnr)/2.0
    if (ppv+tpr)>0.0:
...
def evaluate(fingerprintMethod,similarityMethod,similarityThreshold=0.5,debug=0,morganFpRadius=2,evaluationMode="top"):
    ...
    smiles_test=list(type1_2_test.smiles)+list(type2_test.smiles)
    ...
    nCorrectTotal=0
    nNotFound=0
    confusionDict={"type1_2_ok":0,"type1_2_false":0,"type2_ok":0,"type2_false":0}
